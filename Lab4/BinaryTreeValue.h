...
public:
    ...
    Node<T>* AddNode(Node<T>* node, T data, Node<T>* parent) {
        if (node == nullptr) {
    ...
    void Add(T data) {
        Node<T>* start = root;
        start = AddNode(start, data, nullptr);
    }
    Node<T>* SearchElement(Node<T>* node, T data) {
        if(node->data == data) {
            return node;
        }
        else if(data < node->data) {
    ...
    bool SearchItem(Node<T>* node, T data) {
